...
func ping() error {
	parameter := fmt.Sprintf("root:password@tcp(%s:13306)/?parseTime=true", databaseHost)
...
func connectAndMigrate() (*gorm.DB, error) {
	...
	parameter := fmt.Sprintf("root:password@tcp(%s:13306)/%s?parseTime=true&multiStatements=true", databaseHost, databaseName)
